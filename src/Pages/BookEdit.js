...
const formValidationSchema1 = yup.object({
  name : yup
    .string()
...
function EditForm({book}){
    ...
    const formik = useFormik({
        initialValues     : {name : book.name, author: book.author, year: book.year, rating: book.rating, image: book.image, summary : book.summary, videoSummary : book.videoSummary},
         validationSchema : formValidationSchema1,
          onSubmit        : (values)=> {
            const updatedBook = {
                name         : values.name,
